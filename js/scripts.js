...
// Detección de navegadores Xiaomi y HeyTap
const isXiaomi = /miuibrowser|miui|miwebview/.test(navigator.userAgent.toLowerCase());
const isHeyTap = /heytap/.test(navigator.userAgent.toLowerCase()); // Detección de HeyTap

if (isXiaomi || isHeyTap) { // Si es Xiaomi o HeyTap
  unsupported.style.display = "flex";  // Mostrar mensaje de navegador no soportado
  videos.forEach(v => v.style.display = "none");  // Ocultar videos
  audio.remove();  // Eliminar el audio
  buttonsContainer.style.display = "none";  // Ocultar botones
} else {
